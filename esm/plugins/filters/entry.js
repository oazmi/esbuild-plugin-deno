 ...
 * @module
*/
import { isString, normalizePath, resolveAsUrl } from "../../deps.js";
...
export const entryPluginSetup = (config) => {
    const { filters, pluginData: _initialPluginData, captureDependencies, acceptNamespaces: _acceptNamespaces } = { ...defaultEntryPluginSetup, ...config }, acceptNamespaces = new Set([..._acceptNamespaces, PLUGIN_NAMESPACE.LOADER_HTTP]), captured_resolved_paths = new Set(), ALREADY_CAPTURED_BY_INJECTOR = Symbol(), ALREADY_CAPTURED_BY_RESOLVER = Symbol();
    return async (build) => {
