 ...
 * @module
*/
import { DEBUG, defaultFetchConfig, ensureEndSlash, ensureStartDotSlash, joinPaths, json_stringify, pathToPosixPath, resolveAsUrl } from "../deps.js";
...
*/
export const onResolveFactory = (config) => {
...
*/
export const unResolveFactory = (config, build) => {
    ...
    return async (args) => {
        const { path, namespace: _0, pluginData: { importMap, runtimePackage, originalNamespace: namespace, ...restPluginData } = {}, ...rest_args } = args;
        ...
        const { path: resolved_abs_path, namespace: _1, pluginData: _2, ...rest_resolved_args } = await on_resolve_fn({ path, namespace, ...rest_args, pluginData: { importMap, runtimePackage } });
